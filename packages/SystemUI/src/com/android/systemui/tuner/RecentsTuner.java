...
public class RecentsTuner extends Fragment implements TunerService.Tunable {
    ...
    private void notifyChanged() {
        mPreview.onTuningChanged(NAV_BAR_VIEWS, mRecentsAdapter.getNavString());
    }
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        mRecentsAdapter = new RecentsAdapter(context);
        recyclerView.setAdapter(mRecentsAdapter);
        recyclerView.addItemDecoration(new Dividers(context));
        final ItemTouchHelper itemTouchHelper = new ItemTouchHelper(mRecentsAdapter.mCallbacks);
        mRecentsAdapter.setTouchHelper(itemTouchHelper);
    ...
    public void onTuningChanged(String key, String navLayout) {
        ...
        String[] groups = new String[] { RecentsAdapter.START, RecentsAdapter.CENTER,
                RecentsAdapter.END};
        ...
        for (int i = 0; i < 3; i++) {
            mRecentsAdapter.addButton(groups[i], groupLabels[i]);
            for (String button : views[i].split(BUTTON_SEPARATOR)) {
                mRecentsAdapter.addButton(button, getLabel(button, context));
            }
        }
        mRecentsAdapter.addButton(RecentsAdapter.ADD, getString(R.string.add_button));
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        if (item.getItemId() == SAVE) {
            if (!mRecentsAdapter.hasHomeButton()) {
                ...
                Settings.Secure.putString(getContext().getContentResolver(),
                        NAV_BAR_VIEWS, mRecentsAdapter.getNavString());
            }
            return true;
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == READ_REQUEST && resultCode == Activity.RESULT_OK && data != null) {
            ...
            mRecentsAdapter.onImageSelected(uri);
        } else {
            super.onActivityResult(requestCode, resultCode, data);
        }
    }

    private class RecentsAdapter extends RecyclerView.Adapter<Holder>
